...
│   │   │   │   │   └── StepDone.tsx
│   │   │   │   ├── Navigation.tsx          # Responsive nav: desktop 4-tab hub bar (Plans/Recipes/Preferences/Developer) with active detection via pathname.startsWith(); mobile shows logo + ThemePicker only (hamburger drawer removed — replaced by MobileTabBar)
...
│   │   │   │   ├── AddRecipeModal.tsx      # Modal to add recipe to meal plan (collection filter, advanced filters, defaultDate)
...
- **Meal swapping:** Request 3 alternatives for any meal slot. Alternatives include existing library recipes or new AI suggestions with estimated nutrition. **Browse Library** option in swap popover lets users pick any recipe from the full library via RecipePicker modal (Recipe → SwapAlternative adapter mapping).
...
| MealPlan | meal_plans | name, startDate, endDate, status, numberOfPersons | Status: active/completed/deleted. numberOfPersons (default 1) for family meal planning — nutrition summary divides by this. |
...
| MealPlanPreference | meal_plan_preferences | userId, profileName, dietaryRestrictions, allergies, cuisinePreferences, caloriesMin/Max, proteinPercent, carbsPercent, fatPercent, recipeSource, sourceCollectionId, mealVariety, durationWeeks, durationDays, repeatWeekly, includedMeals, maxPrepTimeWeekday/Weekend, maxCookTimeWeekday/Weekend, cookDaysPerWeek, quickMealMaxMinutes, defaultServings, numberOfPersons, cookingFreeDays, preferredMethods, season | AI meal plan preferences profile. `defaultServings` is auto-set from `numberOfPersons` (no separate UI) — 1 serving per person. |
...
**Diet Goal Calculator** - Self-contained frontend-only component (`DietGoalCalculator.tsx`) that calculates TDEE using the Mifflin-St Jeor equation (BMR = 10×weight + 6.25×height − 5×age ± gender constant, × activity multiplier). Goal presets: Lose weight (−500 kcal, 30/40/30 macros), Maintain (0, 25/50/25), Build muscle (+300, 35/40/25). Hidden by default behind "Need help figuring out your targets?" link in PreferenceEditPage's Nutrition Targets section. On Apply, fills existing calorie and macro form fields — no backend, no database, no persistence. **Family mode:** Toggle "Family" to add multiple members with individual TDEE calculations; outputs per-person average calorie/macro targets for the group.

**Number of Persons for Family Meal Planning** — `numberOfPersons` field on MealPlan (Int, default 1) and MealPlanPreference models. Persons stepper (1-25) in meal plan creation modal, preferences, and AI wizard. MealPlanDetailPage shows inline persons editor (pencil icon → −/+ stepper → done). AddRecipeModal defaults servings to the plan's `numberOfPersons`. Nutrition summary and daily breakdown divide all values by `numberOfPersons` to show per-person intake. AI prompt includes persons context for family-aware meal generation. `defaultServings` field still exists in model but is auto-synced to `numberOfPersons` (no separate UI) — 1 serving per person assumed, user can override per-meal. DietGoalCalculator gained "Family" mode for multi-person TDEE averaging. Key files: `DietGoalCalculator.tsx` (Family mode), `MealPlanDetailPage.tsx` (inline editor + per-person nutrition), `MealPlansPage.tsx` (create modal stepper), `AddRecipeModal.tsx` (servings default), `StepPlanSetup.tsx` (preference field), `PreferenceEditPage.tsx` (preference field), `aiMealPlan.service.ts` (AI prompt context), `mealPlan.service.ts` (create/update), `mealPlanPreference.service.ts` (create/update).
...
**UX Improvements — 11 Changes Across 3 Use Cases** - Comprehensive UX overhaul addressing context loss during navigation, dead-end features, and hidden functionality across three core user journeys (Plan My Week, Build Recipe Library, Quick Grocery Run). All changes are frontend-only, reusing existing backend endpoints. Organized in 5 phases:

- **1A. "Keep Open" Toggle on AddRecipeModal** — `keepOpen` boolean state with checkbox "Keep open to add more". On success: if keepOpen, resets modal to browse step (clears selectedRecipe/quickAdd, advances date +1 day, keeps mealType, shows success toast) instead of closing. Persists across adds so user toggles once and keeps adding. Key file: `AddRecipeModal.tsx`

- **1B. Shopping List Button Stays on Page** — MealPlanDetailPage's "Generate Shopping List" success handler changed from `navigate()` to `toast.success()` with clickable link: "Shopping list created! View list →". Uses react-hot-toast's JSX support. Same pattern for "Add to existing list". Dropdown closes after action. Key file: `MealPlanDetailPage.tsx`

- **1C. Auto-Navigate to New Shopping List** — ShoppingListsPage's 4 creation callbacks (generate from plans, generate from recipes, custom, split) changed from `mutate` to `mutateAsync` to capture returned `ShoppingList.id`, then `navigate(`/shopping-lists/${list.id}`)`. For split lists (returns array), navigates to the first list. No hook changes needed. Key file: `ShoppingListsPage.tsx`

- **1D. "Create Cooking Plan" Button on MealPlanDetailPage** — `CookingPot` icon button in header action buttons row. `onClick`: navigates to `/cooking-plan/new?planId=${id}`. CookingPlanPage reads `searchParams.get('planId')` on mount, pre-selects that meal plan in the cooking plan wizard via `useEffect`. Key files: `MealPlanDetailPage.tsx`, `CookingPlanPage.tsx`

- **2A. AddToShoppingListModal Component** — New modal (`AddToShoppingListModal.tsx`) mirroring `AddToMealPlanModal` pattern. Props: `recipeId, recipeName, isOpen, onClose`. Two modes: "Create new list" (name field pre-filled, calls `generateFromRecipes.mutateAsync`) and "Add to existing list" (dropdown of active lists via `useShoppingLists('active')`, calls `shoppingListsService.addFromRecipes`). Both navigate to the list on success.

- **2B. Shopping List Button on Recipe Pages** — `ShoppingCart` icon button added to RecipeDetailPage header (between Collection and Edit) and RecipesPage recipe cards. Both trigger `AddToShoppingListModal` with recipe ID and title. Key files: `RecipeDetailPage.tsx`, `RecipesPage.tsx`

- **3A. Bulk Actions on RecipesPage** — "Select to Delete" button renamed to "Bulk Actions". Selected recipes show action bar with 4 options: Add to Collection (opens `AddToCollectionModal` with first selected recipe, batch add via `collectionsService.addRecipesBatch`), Shopping List (calls `generateFromRecipes.mutateAsync` with all selected IDs), Delete (existing), Cancel. Reuses existing checkbox selection UX. Key file: `RecipesPage.tsx`

- **3B. Tag Manager Link on RecipesPage** — `Tag` icon button in RecipesPage header (near AI Generate) linking to `/developer/tags`. TagManagerPage back link changed from "← Back to Developer Tools" to "← Back to Recipes" pointing to `/recipes`. Developer page card kept as secondary access. Key files: `RecipesPage.tsx`, `TagManagerPage.tsx`

- **3C. ShoppingListBuilder Tag Filter** — Added tag filter chips above recipe checkboxes in Recipes tab. Uses `TAG_CATEGORIES` from `tagDefinitions.ts`. Filter: `recipe.tags.includes(selectedTag)`. Added "X selected" counter next to tab title and "Select all shown" / "Clear selection" quick action links. Key file: `ShoppingListBuilder.tsx`

- **4A. UrlImportPage SessionStorage Persistence** — Follows `AIRecipeGeneratorPage` sessionStorage pattern. Scraped results saved to `sessionStorage` on change, restored on mount via lazy `useState` initializer. Tracks imported recipe indices (`importedIndices: Set<number>`) also in sessionStorage. Imported recipes shown with green checkmark + "Imported" badge, dimmed card. "Clear session" button resets state. Review & Import passes `{ source: 'urlImport', importIndex }` in location.state. Key file: `UrlImportPage.tsx`

- **4B. Return-to-Import Navigation** — RecipeFormPage captures `location.state.source` and `importIndex`. After successful recipe create, if `source === 'urlImport'`: navigates back to `/recipes/import-urls` with `{ importedIndex }` in state + success toast. UrlImportPage reads `importedIndex` on remount and marks that recipe as imported. Key file: `RecipeFormPage.tsx`

- **5A. WeekGridView Component** — New compact 7-column week grid (`WeekGridView.tsx`) for meal plan visualization. Shows one week at a time (Mon–Sun), each cell with day header + sorted meal list (colored dots per meal type + truncated recipe title links). Week navigation with prev/next arrows, disabled at plan boundaries. Auto-selects today's week if within plan range. `viewMode: 'cards' | 'grid'` toggle on MealPlanDetailPage with `LayoutList` / `Grid3X3` icons. Responsive: `min-w-[700px]` with horizontal scroll on mobile. Reuses `MEAL_TYPE_COLORS` from `MealPlanCalendar.tsx`. Click day header → `onDateClick` (scrolls to day card). Key files: `WeekGridView.tsx` (NEW), `MealPlanDetailPage.tsx`

- **AddRecipeModal defaultDate Fix** — Fixed bug where pressing the per-day "+" button on MealPlanDetailPage always showed today's date instead of the clicked day's date. Root cause: `useEffect` only handled `!isOpen` (close/reset), never applied `defaultDate` when opening. Fix: added `isOpen` branch to set date from `defaultDate` prop. Key file: `AddRecipeModal.tsx`

**AddRecipeModal Collection & Advanced Filters** — Enhanced recipe picker modal with the same filtering capabilities as RecipesPage. Collection dropdown filters recipes to a specific collection (uses `useCollections('active')` for list + `useCollection(id)` for recipe ID set). Collapsible advanced filter panel with 6 inputs: Max Calories, Min Protein, Max Carbs, Max Fat, Max Total Time, Max Prep Time (same client-side `useMemo` logic as RecipesPage). All filters stack: search text + collection + nutrition/time. Result counter shows "Showing X of Y recipes in *Collection Name*" when active. Filter toggle button shows active count badge. All state resets on modal close. CSS grid animation for smooth panel expand/collapse. Key file: `AddRecipeModal.tsx`

**Mobile UX Improvements (5 Changes)** — Five targeted fixes for mobile meal plan workflows:
- **Nutrition summary per-serving:** MealPlanDetailPage nutrition summary now shows daily average as primary big number (e.g., "1278 Calories / day") with plan total as secondary small text below. Previously showed plan total as primary, which was less actionable for daily nutrition tracking.
- **Servings stepper buttons:** `ServingsStepper` component in `AddRecipeModal.tsx` replaces `<input type="number">` with `−` / `+` tap-friendly buttons (w-9 h-9 on detail form, w-7 h-7 on quick-add). Minimum 1 serving enforced. Used in both quick-add inline form (Step 1) and detail form (Step 2).
- **Create button keyboard visibility:** MealPlansPage create modal moved Cancel/Create buttons from `footer` prop into the modal body (below input with `mt-6`), so they scroll with content instead of being hidden behind the mobile keyboard.
- **Redirect to new meal plan:** `handleCreateWeeklyPlan` changed from `mutate` to `mutateAsync` to capture returned plan ID, then `navigate(`/meal-plans/${newPlan.id}`)` redirects user directly into the new empty meal plan.
- **Print text size increase:** Shopping list print styles bumped up: container `print:text-base`, categories `print:text-lg`, ingredient names `print:text-base`, quantities `print:text-sm`, checkboxes `print:text-lg`, padding `print:p-3`. Previously used `print:text-sm`/`print:text-xs` which were too small on phone PDFs.

**Meal Plan Rename** — Pencil icon button next to meal plan name on MealPlanDetailPage opens rename modal (Input + Save/Cancel). Uses existing `useUpdateMealPlan` hook — no backend changes needed. Consistent with rename patterns on shopping lists and cooking plans.

**Browse Library in Swap Alternatives** — StepReviewPlan swap popover now includes a "Browse full library..." button (with Library icon) below AI alternatives. Opens RecipePicker modal; selected recipe is mapped from `Recipe` type to `SwapAlternative` via adapter function (`applyLibrarySwap`). Swap alternative cards use `flex-wrap items-start` for mobile title wrapping. Main meal cards restructured to two-row layout: row 1 = meal type badge + title (wraps freely, no `truncate`) + swap button; row 2 = AI/Library label + calories + cook time — prevents title clipping on mobile.

**Collection-Only Recipe Source** — Third recipe source option (`collection_only`) alongside `library_only` and `library_and_ai`. When selected, user picks a specific recipe collection; only recipes from that collection are used for AI plan generation. Full stack: Prisma `sourceCollectionId` field on MealPlanPreference, Zod validator updated, `getRecipeLibrarySummary()` in `aiMealPlan.service.ts` filters by collection via RecipeCollectionItem junction table, `generatePlan()` and `swapMeal()` pass collectionId. UI: third button + collection dropdown in StepPlanSetup and PreferenceEditPage, validation disables Generate when no collection selected. PreferencesPage badge shows "Collection" with orange variant.

**AI Create in AddRecipeModal** — Emerald "AI Create" button (Sparkles icon) in AddRecipeModal toolbar row (next to Filters), always visible. Navigates to `/recipes/ai-generate?mealPlanId=xxx` passing meal plan context. AIRecipeGeneratorPage reads and persists `mealPlanId` via URL params + sessionStorage. After AI recipe creation, user is redirected back to meal plan detail page with AddRecipeModal auto-opened and new recipe pre-selected via `location.state.addRecipeId`. Full round-trip: MealPlan → AI Wizard → RecipeFormPage → back to MealPlan with recipe ready to add. Key files: `AddRecipeModal.tsx` (button + `preSelectedRecipeId` prop), `AIRecipeGeneratorPage.tsx` (mealPlanId persistence), `StepReviewCreate.tsx` (pass mealPlanId), `StepDone.tsx` ("Back to Meal Plan" button), `RecipeFormPage.tsx` (meal plan redirect on save), `MealPlanDetailPage.tsx` (auto-open modal on return).
...
## AI Recipe/Meal Plan Two-Stage Pipeline

The AI generation flow has two stages: **Stage 1** (suggest — `POST /api/ai-recipes/generate` or `POST /api/ai-meal-plan/generate`) receives all user preferences. **Stage 2** (generate full recipe details — `POST /api/ai-meal-plan/generate-recipe-details`) recreates the recipe from scratch and has **NO memory of stage 1 preferences** unless explicitly passed.

**When adding a new preference field**, it must be threaded through ALL of these files:
- Backend: `mealPlanPreference.validator.ts` (add to `generateRecipeDetailsSchema`) → `aiMealPlan.service.ts` (`generateRecipeDetails()` signature + prompt) → `aiMealPlan.controller.ts` (destructure + pass)
- Frontend: `aiMealPlan.service.ts` (`GenerateRecipeDetailsParams` interface) → `useAIMealPlan.ts` (hook mutation) → `StepReviewCreate.tsx` (AI Recipe wizard) + `StepCreateRecipes.tsx` (AI Meal Plan wizard) → `AIRecipeGeneratorPage.tsx` (pass `generationInput` prop)

**Prompt effectiveness:** Abstract restriction labels ("dairy-free") are weaker than explicit forbidden ingredient lists. The `generateRecipeDetails()` service builds a concrete `FORBIDDEN INGREDIENTS` list from restrictions/allergies — extend the `forbiddenIngredients` arrays there when adding new restriction types.
